...
def execute(prog,reg):
    ...
    while ip<len(prog):
        ...
        comb=combo(op2,reg)
        if op1==0: num,denom=reg[0],2**comb;reg[0]=num//denom
        if op1==1: reg[1]=op2^reg[1]
        if op1==2: reg[1]=comb%8
        if op1==3 and reg[0]!=0: ip=op2; continue
        if op1==4: reg[1]=reg[1]^reg[2]
        if op1==5: output.append(comb%8)
        if op1==6: num,denom=reg[0],2**comb;reg[1]=num//denom
        if op1==7: num,denom=reg[0],2**comb;reg[2]=num//denom
        ip+=2
    return output
...
print("".join(map(str,part1)),part2)